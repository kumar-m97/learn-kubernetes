...
Kubelet:(H3) The Kubelet is the agent that runs on each Worker Node and communicates with the Master Node. It ensures that the containers described in the pod specifications are running and healthy.

Container Runtime:(H3) Kubernetes supports multiple container runtimes, such as Docker or containerd. The Container Runtime is responsible for pulling container images and running containers on the Worker Nodes.

Kube Proxy:(H3) Kube Proxy is responsible for network communication within the cluster. It manages the network routing for services and performs load balancing.

🚀 When a user deploys a new application in Kubernetes — What happens?
Step 1:(H4) User submits Deployment YAML ---- kubectl apply -f deployment.yaml
...
Step 2:(H4) Scheduler decides where to run Pods
...
Step 3:(H4) Kubelet on Worker Node creates Pod
...
Step 4:(H4) Pod status updated and monitored
...
Main K8s Components & Objects(H2)
Nodes:(H3)
...
Basic File Structure of Any Kubernetes Manifest(H3)
