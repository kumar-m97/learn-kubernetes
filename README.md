...
### Kubelet: 
The Kubelet is the agent that runs on each Worker Node and communicates with the Master Node. It ensures that the containers described in the pod specifications are running and healthy.

### Container Runtime:
Kubernetes supports multiple container runtimes, such as Docker or containerd. The Container Runtime is responsible for pulling container images and running containers on the Worker Nodes.

### Kube Proxy:
Kube Proxy is responsible for network communication within the cluster. It manages the network routing for services and performs load balancing.

## 🚀 When a user deploys a new application in Kubernetes — What happens?
### Step 1: 
User submits Deployment YAML ---- kubectl apply -f deployment.yaml
...
### Step 3:
Kubelet on Worker Node creates Pod
...
### Step 4:
Pod status updated and monitored
